...
object NewInMemoryLshTable {
    def createTables( numTables : Int, prefix : Option[ String ] = None ) : IndexedSeq[ NewLshTable ] = {
        for {
            _ <- 1 to numTables
        } yield new NewInMemoryLshTable( prefix )
    }
}

class NewInMemoryLshTable( override protected val prefix : Option[ String ] = None ) extends NewLshTable {
    ...
    override def put( entry : LshEntry ) : Unit = {
        ...
        if ( !index.keySet.contains( value.hash ) ) index( value.hash ) = new HashSet[ String ]()
    ...
    override def update( entry : LshEntry ) : Unit = {
        ...
        if ( newValue.hash != oldValue.hash ) {
            ...
            if ( !index.keySet.contains( newValue.hash ) ) index( newValue.hash ) = new mutable.HashSet[ String ]()
