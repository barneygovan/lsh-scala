...
trait JsonSerialization extends Serialization {
    ...
    override def deserialize( input : Array[ Byte ] ) : LshEntry = {
        ...
        val point : DenseVector[ Double ] = {
            val node : ArrayNode = json.get( "point" ).withArray[ ArrayNode ]( "data$mcD$sp" )
            val vector =
                node
                  .elements().asScala
                  .flatMap {
                      case node : DoubleNode => Some( node.doubleValue() )
                      case other => throw new IllegalStateException( s"non numeric value found in Array[ Double ] - ${other.toString}" )
                  }.toArray
            DenseVector( vector )
