...
class DistanceFunctionSpec extends AnyFunSpec with Matchers {
  describe("calculating Euclidean distance score") {
    it(
      ...
      euclideanDistance(point1, point2) should equal(
        1.0 / (1.0 + Math.sqrt(2.0))
      )
      euclideanDistance(point1, point3) should equal(
        1.0 / (1.0 + Math.sqrt(4.0))
      )
      euclideanDistance(point2, point3) should equal(
        1.0 / (1.0 + Math.sqrt(10.0))
      )
    }
  }
